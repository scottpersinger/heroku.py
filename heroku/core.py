...
def from_access_token(access_token, refresh_token = None, client_secret=None, version=None):
    """Returns a Heroku instance authenticated from an Oauth access token."""
    h = Heroku(version=version)
    h.authenticate_oauth(access_token, refresh_token=refresh_token, oauth_secret=client_secret)
    return h